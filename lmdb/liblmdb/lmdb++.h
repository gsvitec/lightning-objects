...
#if defined(_MSC_VER)
#if _MSC_VER < 1800
#error "<lmdb++.h> requires a Visual Studio 2012 or higher"
#endif
#if _MSC_VER < 1900
#define MSVC12_FIX
#define noexcept
...
public:
  fatal_error(const char* const origin, const int rc) : error(origin, rc) {}
  //using error::error;
};
...
public:
  runtime_error(const char* const origin, const int rc) : error(origin, rc) {}
  //using error::error;
};
...
public:
  key_exist_error(const char* const origin, const int rc) : runtime_error(origin, rc) {}
...
public:
  not_found_error(const char* const origin, const int rc) : runtime_error(origin, rc) {}
  //using runtime_error::runtime_error;
};
...
public:
  corrupted_error(const char* const origin, const int rc) : fatal_error(origin, rc) {}
...
public:
  panic_error(const char* const origin, const int rc) : fatal_error(origin, rc) {}
  //using fatal_error::fatal_error;
};
...
public:
  version_mismatch_error(const char* const origin, const int rc) : fatal_error(origin, rc) {}
  //using fatal_error::fatal_error;
};
...
public:
  map_full_error(const char* const origin, const int rc) : runtime_error(origin, rc) {}
  //using runtime_error::runtime_error;
};
...
public:
  bad_dbi_error(const char* const origin, const int rc) : runtime_error(origin, rc) {}
  //using runtime_error::runtime_error;
};
...
public:
   ...
   * Move constructor.
   */
  val(val&& other) : _val(std::move(other._val)) {}

  /**
   * Move assignment operator.
   */
  val& operator=(val&& other) {_val = std::move(other._val);}
#else
  /**
   * Move constructor.
   */
...
public:
  ...
  /**
   * Sets a custom dupsort value comparison function for this database.
   *
   * @param txn a transaction handle
   * @param cmp the comparison function
   * @throws lmdb::error on failure
