...
public:
  ...
  {
    ClassTraits<T>::traits_info->refcounting = refcount;
  }
...
protected:
  ...
  template<typename T> T *loadObject(ClassId classId, ObjectId objectId)
  {
    ...
    T *obj = ClassTraits<T>::makeObject(classId);
  ...
  template<typename T> T *loadObject(ObjectKey &key, T *obj=nullptr)
  {
    ...
    if(!obj) obj = ClassTraits<T>::makeObject(key.classId);
...
protected:
  ...
  /**
   * save a data buffer under an object key
   */
...
public:
  ...
  template <typename T>
  void deleteObject(T *obj, ObjectKey &key) {
...
template<typename T, typename V> struct ObjectPropertyStorage : public StoreAccessEmbeddedKey
{
  ...
  size_t prepareDelete(WriteTransaction *tr, ObjectBuf &buf, const PropertyAccessBase *pa) override
  {
    ...
    if(refcount) {
      ...
      ObjectId oid;
      buf.read(cid, oid);
      if(tr->decrementRefCount(cid, oid) == 0) tr->removeObject<V>(cid, oid);
    }
    return size(buf);
  }
